...
namespace ACF_Widgets\Classes\Activate;
...
class Activate {
	...
	public function __construct() {
		...
		if ( ! Classes\acfw_php()->version() ) {
			...
			add_action( 'after_plugin_row_' . ACFW_BASENAME, [ $this, 'php_deactivate_notice_row' ], 5, 3 );
	...
	public function php_deactivate_notice_row( $plugin_file, $plugin_data, $status ) {
		...
		?>
		<style>
			.plugins tr[data-plugin='<?php echo ACFW_BASENAME; ?>'] th,
			.plugins tr[data-plugin='<?php echo ACFW_BASENAME; ?>'] td {
				box-shadow: none;
			}

			<?php if ( isset( $plugin_data['update'] ) && ! empty( $plugin_data['update'] ) ) : ?>

				.plugins tr.<?php echo 'acf-widgets'; ?>-plugin-tr td {
					box-shadow: none ! important;
				}

				.plugins tr.<?php echo 'acf-widgets'; ?>-plugin-tr .update-message {
					margin-bottom: 0;
				}
		...
		<tr id="plugin-php-notice" class="plugin-update-tr active <?php echo 'acf-widgets'; ?>-plugin-tr">
			<td colspan="<?php echo $colspan; ?>" class="plugin-update colspanchange">
				<div class="update-message notice inline notice-error notice-alt">
					<?php echo sprintf(
						'<p>%s %s %s %s %s %s</p>',
						__( 'Functionality of the', 'acf-widgets' ),
						ACFW_NAME,
						__( 'plugin has been disabled because it requires PHP version', 'acf-widgets' ),
						Classes\acfw_php()->minimum(),
						__( 'or greater. Your system is running PHP version', 'acf-widgets' ),
						phpversion()
					); ?>
				</div>
			</td>
