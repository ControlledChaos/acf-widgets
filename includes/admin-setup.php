...
function acfw_lang(){
	load_plugin_textdomain('acf-widgets', false, dirname(plugin_basename( __FILE__ ) ) .'/lang');
} // end acfw_lang()
...
function acfw_register_cpt() {
	...
	register_post_type('acf-widgets', array(
		'label' => __('ACF Widgets', 'acf-widgets'),
		'description' => __('Widgets created with the ACF Widgets Plugin.', 'acf-widgets'),
		'public' => false,
		'show_ui' => true,
		...
		'labels' => array (
			'name' => __('Widgets', 'acf-widgets'),
			'singular_name' => __('ACF Widget', 'acf-widgets'),
			'menu_name' => __('Add New Widgets', 'acf-widgets'),
			'add_new' => __('Add Widget', 'acf-widgets'),
			'add_new_item' => __('Add New Widget', 'acf-widgets'),
			'edit' => __('Edit', 'acf-widgets'),
			'edit_item' => __('Edit Widget', 'acf-widgets'),
			'new_item' => __('New Widget', 'acf-widgets'),
			'view' => __('View Widget', 'acf-widgets'),
			'view_item' => __('View Widget', 'acf-widgets'),
			'search_items' => __('Search Widgets', 'acf-widgets'),
			'not_found' => __('No Widgets Found', 'acf-widgets'),
			'not_found_in_trash' => __('No Widgets Found in Trash', 'acf-widgets'),
			'parent' => __('Parent Widget', 'acf-widgets'),
		)
	) ); 
...
function acfw_widget_meta_boxes() {
	add_meta_box(
		'acfw-helper-text',
		__( 'Using ACF Widgets in Your Theme', 'acf-widgets' ),
...
function acfw_how_to_meta_box( $post ) {
	...
	if ($title == ''){
		_e('For more information, give your widget a title, then Publish or Update this page.', 'acf-widgets');
	} else {
...
function acfw_support_meta_box(){
	...
	add_meta_box('acfw-support', __('Support', 'acf-widgets'), 'acfw_support_meta_box_html', 'acf-widgets', 'side');
		...
		if ( wp_verify_nonce( $_POST['_wpnonce'], 'acfw_options_nonce' ) ){
			...
			if ( isset( $_POST['deactivate'] ) ){
				...
				if ( $status->success === false && $status->error === 'no_activations_left' ){
					$error_message = __( 'You have no activations remaining.', 'acf-widgets' );
				}
...
function acfw_add_columns($columns){
	unset($columns['date']);
	$columns['template'] = __('Theme Template', 'acf-widgets');
	return $columns;
} // End custom columns
...
function acfw_cpt_title( $title ){
	...
	if ('acf-widgets' == $screen->post_type){
		$title = __('Enter widget name', 'acf-widgets' );
	} return $title;
} // End acfw_cpt_title()
...
function acfw_custom_excerpt_title( $input ) {
    ...
    if ( is_admin() && 'acf-widgets' == $post_type && 'Excerpt' == $input ){
        return __( 'Widget Description', 'acf-widgets' );
    }
    return $input;
...
function acfw_lite_css(){
	...
	if ( defined('ACFW_LITE') )
		echo "<style> .acf-field[data-key*='acf-widgets']{ display: none; } </style>";
